...
usage() {
  cat <<HERE
Usage:
  build.sh --from-release --flink-version 1.9.0 --scala-version 2.11 --hadoop-version 2.7 --image-name gary0416/flink:1.9.0-hadoop27-scala_2.11-alpine
...
if [ -n "${FROM_RELEASE}" ]; then
  ...
  # After Flink-1.8 we would let release pre-built package with hadoop
    if [[ -n "${HADOOP_VERSION}" ]]; then
        ...
        # https://repo.maven.apache.org/maven2/org/apache/flink/flink-shaded-hadoop2-uber/未更新1.9版本的，改用官网文档中提供的地址下载
        DOWNLOAD_SHADED_HADOOP_URL=$(curl -v --silent https://flink.apache.org/downloads.html 2>&1 | grep flink-shaded-hadoop-2-uber-${HADOOP_VERSION} | head -1 | sed 's/.*="\(.*\).sha1".*/\1/')
        checkUrlAvailable ${DOWNLOAD_SHADED_HADOOP_URL}

        SHADED_HADOOP_FILE_NAME=`echo $DOWNLOAD_SHADED_HADOOP_URL | sed 's/.*\///'`
